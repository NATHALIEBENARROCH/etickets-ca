...
function validateEnv() {
  const requiredValues = {
    TN_CONSUMER_KEY: resolveEnv('TN_CONSUMER_KEY', 'TICKETNETWORK_CONSUMER_KEY'),
    TN_CONSUMER_SECRET: resolveEnv('TN_CONSUMER_SECRET', 'TICKETNETWORK_CONSUMER_SECRET'),
  };
...
async function fetchTokenFromServer() {
  ...
  const key = resolveEnv('TN_CONSUMER_KEY', 'TICKETNETWORK_CONSUMER_KEY');
  const secret = resolveEnv('TN_CONSUMER_SECRET', 'TICKETNETWORK_CONSUMER_SECRET');
  const tokenUrl = resolveEnv('TN_TOKEN_URL') || 'https://key-manager.tn-apis.com/oauth2/token';
...
export function buildGetEventsSOAP(params = {}) {
  const {
    websiteConfigID = resolveEnv('TN_WCID', 'WCID'),
    numberOfEvents,
    venueName,
    eventDateFrom,
    eventDateTo,
...
export async function callGetEvents(params = {}) {
  ...
  const apiUrl = resolveEnv('TN_API_URL', 'TN_API_BASE') || 'https://www.tn-apis.com/TNWebservices/v4/TNWebservice.asmx';
