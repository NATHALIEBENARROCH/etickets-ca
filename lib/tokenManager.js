...
function validateEnv() {
  const required = [
    ['TN_CONSUMER_KEY', 'TICKETNETWORK_CONSUMER_KEY'],
    ['TN_CONSUMER_SECRET', 'TICKETNETWORK_CONSUMER_SECRET'],
    ['TN_TOKEN_URL', 'TICKETNETWORK_TOKEN_URL'],
  ];
  const missing = required.filter(group => !getEnv(...group));
  if (missing.length > 0) {
    throw new Error(`Missing required env vars: ${missing.map(group => group.join(' or ')).join(', ')}`);
  }
}
...
async function fetchTokenFromServer() {
  ...
  const key = getEnv('TN_CONSUMER_KEY', 'TICKETNETWORK_CONSUMER_KEY');
  const secret = getEnv('TN_CONSUMER_SECRET', 'TICKETNETWORK_CONSUMER_SECRET');
  const tokenUrl = getEnv('TN_TOKEN_URL', 'TICKETNETWORK_TOKEN_URL');
...
export function buildGetEventsSOAP(params = {}) {
  const {
    websiteConfigID = getEnv('TN_WCID', 'WCID'),
    numberOfEvents,
    venueName,
    eventDateFrom,
    eventDateTo,
...
export async function callGetEvents(params = {}) {
  ...
  const apiUrl = getEnv('TN_API_URL', 'TICKETNETWORK_API_URL') || getEnv('TN_API_BASE', 'TICKETNETWORK_API_BASE');
